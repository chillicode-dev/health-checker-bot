module.exports = {
  ...
  `,
  addProjectWrongArgs: () => `
    ⚠ Укажите проект для добавления и список адресов для проверки через запятую.\nНапример: '/add project-name http://example1.com,http://example2.com'
  `,
  addProjectWrongUrls: () => `
  ...
  `,
  deleteProjectFail: () => `
    ⚠ Ошибка удаления. Проверьте название проекта
  `,
  ...
  listOfProjectsNull: () => `
    ⚠ Не найдено ни одного проекта. Используйте команду '/add project-name http://example1.com,http://example2.com' для добавления проекта
  `,
  startSuccess: () => `
    🚀 Сервис пинга запущен
  `,
