...
function addProjectToChat(telegramChatId, project) {
  return Chat.updateOne({telegramChatId}, {$push: {projects: project}}).exec();
}

function deleteProjectFromChat(telegramChatId, projectName) {
  return Chat.updateOne({telegramChatId}, {$pull: {projects: {name: projectName}}}).exec();
}
